...
class Command(BaseCommand):
    help = 'Processes pending FCM push notifications from milestone_backend_notification collection in MongoDB'
    ...
    def handle(self, *args, **options):
        ...
        if is_daemon:
            self.stdout.write(self.style.SUCCESS(f"Starting Notification Background Worker Daemon (Polling every {interval}s)..."))
            tick = 0
            while True:
                try:
                    tick += 1
